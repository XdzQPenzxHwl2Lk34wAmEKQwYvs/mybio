...
// Fetch logs for this program and user
$check_logs = mysqli_query($con, "SELECT * FROM logs WHERE PROGRAM='" . mysqli_real_escape_string($con, $_SESSION['program']) . "' AND USER_ID='" . $_SESSION['ID'] . "' ORDER BY created_at DESC");
$res_c = array(); // Use $res_c to match your code snippet variable name
while($log_row = mysqli_fetch_assoc($check_logs)) {
    $res_c[] = $log_row;
}

// Handle Purge Logs action
if(isset($_POST['PurgeLogs'])){
    mysqli_query($con, "DELETE FROM `logs` WHERE `USER_ID` = '" . $_SESSION['ID'] . "' AND `PROGRAM` = '" . mysqli_real_escape_string($con, $_SESSION['program']) . "'");
    fnx("Logs.php");
}
?>
<!DOCTYPE html>
...
<head>
    ...
    <title>Logs - undetect.space</title>
        ...
        <div class="content">
            ...
            <div class="stats-grid">
                ...
                <div class="stat-card">
                    ...
                    <div class="stat-value"><?php echo count($res_c); ?></div>
                    <div class="stat-icon">
                        <i class="fas fa-server"></i>
                    </div>
                </div>
            ...
            <div class="card">
                <div class="card-body">
                    <form method="post" onsubmit="return confirm('Are you sure you want to purge all logs for this program?')">
                        <button type="submit" name="PurgeLogs" class="btn btn-danger">
                            <i class="fas fa-trash-alt"></i> <span>Purge All Logs</span>
                        </button>
                    </form>
                </div>
            </div>
            ...
            <div class="card">
                <div class="card-header">
                    <div class="card-title">Program Logs</div>
                    <div>View and manage system logs for your program</div>
                </div>
                <div class="card-body">
                    <div class="table-responsive">
                        <table>
                            ...
                            <tbody>
                                <?php if (empty($res_c)): ?>
                                    <tr>
                                        <td colspan="5" style="text-align: center; color: var(--text-muted);">No logs found</td>
                                    </tr>
                                <?php else: ?>
                                    <?php foreach ($res_c as $log): ?>
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                <?php else: ?>
                                                    <span class="badge badge-success"><?php echo htmlspecialchars($log['log_level']); ?></span>
                                                <?php endif; ?>
                                            </td>
