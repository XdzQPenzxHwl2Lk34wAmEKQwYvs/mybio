...
// Fetch resellers for this program and user
// This query now uses the PROGRAM column which should exist after running the SQL above
$check_resellers = mysqli_query($con, "SELECT * FROM resellers WHERE PROGRAM='" . mysqli_real_escape_string($con, $_SESSION['program']) . "' AND USER_ID='" . $_SESSION['ID'] . "'");
$res_b = array();
while($reseller_row = mysqli_fetch_assoc($check_resellers)) {
    $res_b[] = $reseller_row;
}
...
if (isset($_POST['reseller'])){
    ...
    $password = md5($_POST['Password']); // Note: Consider using password_hash() for better security
    $balance = floatval($_POST['balance']); // Ensure it's a number

    // Insert into database
    // Make sure column names match your actual database table structure
    $insert_query = "INSERT INTO `resellers`(`USERNAME`, `PASSWORD`, `PROGRAM`, `USER_ID`, `USD Balance`)
                     VALUES ('$username','$password','" . mysqli_real_escape_string($con, $_SESSION['program']) . "','" . $_SESSION['ID'] . "','$balance')";

    if (mysqli_query($con, $insert_query)) {
        fnx("Resellers.php");
    } else {
        echo '<script>alert("Error creating reseller: ' . mysqli_error($con) . '");</script>';
    }
}
...
if (isset($_POST['delete'])){
    // Sanitize the ID
    $delete_id = intval($_POST['delete']); // Use intval for ID
    mysqli_query($con, "DELETE FROM `resellers` WHERE `ID` = $delete_id AND `USER_ID` = '" . $_SESSION['ID'] . "' AND `PROGRAM` = '" . mysqli_real_escape_string($con, $_SESSION['program']) . "'");
    fnx("Resellers.php");
}

// Handle reseller balance update
if (isset($_POST['ban'])){ // Note: The form uses 'ban' for updating balance
    // Sanitize inputs
    $reseller_id = intval($_POST['ban']); // Use intval for ID
    $new_balance = floatval($_POST['reason']); // Ensure it's a number

    mysqli_query($con, "UPDATE `resellers` SET `USD Balance` = '$new_balance' WHERE `ID` = $reseller_id AND `USER_ID` = '" . $_SESSION['ID'] . "' AND `PROGRAM` = '" . mysqli_real_escape_string($con, $_SESSION['program']) . "'");
    fnx("Resellers.php");
}
?>
<!DOCTYPE html>
...
<head>
    ...
    <title>Resellers - undetect.space</title>
...
<body>
    ...
    <div class="sidebar">
        ...
        <div class="nav-links">
            ...
            <div class="nav-item">
                <a href="Resellers.php" class="nav-link active">
    ...
    <div class="main-content">
        ...
        <div class="content">
            <h1 class="page-title">Reseller Management</h1>
            ...
            <div class="card">
                <div class="card-header">
                    <div class="card-title">Create New Reseller</div>
                    <div>Add a new reseller for your program</div>
                </div>
                <div class="card-body">
                    <form method="post">
                        ...
                        <div class="form-group">
                            ...
                            <input type="number" name="balance" id="balance" class="form-control" step="0.01" min="0" value="0" required>
                        </div>

                        <button type="submit" name="reseller" class="btn btn-primary">
                            <i class="fas fa-user-plus"></i> <span>Create Reseller</span>
                        </button>
                    </form>
                </div>
            </div>
            ...
            <div class="card">
                <div class="card-header">
                    ...
                    <div>Manage your program resellers</div>
                </div>
                <div class="card-body">
                    <div class="table-responsive">
                        <table>
                            ...
                            <tbody>
                                <?php if (empty($res_b)): ?>
                                    <tr>
                                        <td colspan="4" style="text-align: center; color: var(--text-muted);">No resellers found</td>
                                    </tr>
                                <?php else: ?>
                                    <?php foreach ($res_b as $reseller): ?>
                                        <tr>
                                            <!-- Safely output reseller data with null checks -->
                                            <td><?php echo isset($reseller['ID']) ? htmlspecialchars($reseller['ID']) : 'N/A'; ?></td>
                                            <td><?php echo isset($reseller['USERNAME']) ? htmlspecialchars($reseller['USERNAME']) : 'N/A'; ?></td>
                                            <td>$<?php echo isset($reseller['USD Balance']) ? htmlspecialchars(number_format((float)$reseller['USD Balance'], 2, '.', '')) : '0.00'; ?></td>
                                            <td>
                                                <div class="actions">
                                                    <!-- Update Balance Form -->
                                                    <form method="post" style="display: inline-flex; align-items: center; gap: 5px;">
                                                        <input type="hidden" name="ban" value="<?php echo isset($reseller['ID']) ? htmlspecialchars($reseller['ID']) : ''; ?>">
                                                        <input type="number" name="reason" value="<?php echo isset($reseller['USD Balance']) ? htmlspecialchars($reseller['USD Balance']) : '0.00'; ?>" step="0.01" min="0" style="width: 100px; padding: 8px 12px; border-radius: 6px; border: 1px solid var(--border); background-color: var(--secondary); color: var(--text); font-family: inherit; font-size: 0.9rem;">
                                                        <button type="submit" class="btn btn-info btn-sm">
                                                            <i class="fas fa-edit"></i> <span>Update</span>
                                                        </button>
                                                    </form>

                                                    <!-- Delete Reseller Form -->
                                                    <form method="post" style="display: inline;" onsubmit="return confirm('Are you sure you want to delete this reseller?')">
                                                        <input type="hidden" name="delete" value="<?php echo isset($reseller['ID']) ? htmlspecialchars($reseller['ID']) : ''; ?>">
