...
$check_program = mysqli_query($con, "SELECT * FROM programs WHERE PROGRAM_USER='" . mysqli_real_escape_string($con, $_SESSION['username']) . "'");
$res_b = array();
while($program_row = mysqli_fetch_assoc($check_program)) {
    $res_b[] = $program_row;
}
...
{
    if ($row[3] == "Starter Plan" && count($res_b) >= 5)
    {
        echo '<script>alert("Starter Plan limit reached (5 programs)"); window.location.replace("index.php");</script>';
        exit();
    }
    if ($row[3] == "Ultimate Plan" && count($res_b) >= 50)
    {
        echo '<script>alert("Ultimate Plan limit reached (50 programs)"); window.location.replace("index.php");</script>';
        exit();
    }
    if ($row[3] == "Unlimited Plan" && count($res_b) >= 500)
    {
        echo '<script>alert("Unlimited Plan limit reached (500 programs)"); window.location.replace("index.php");</script>';
        exit();
    }

    $programid = substr(str_shuffle("ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz1234567890987654321") , 0, 12);
    $insert_query = "INSERT INTO `programs`(`PROGRAM_NAME`, `PROGRAM_ID`, `PROGRAM_USER`) VALUES ('" . mysqli_real_escape_string($con, $_POST['programname']) . "','" . $programid . "','" . $_SESSION['username'] . "')";
...
{
    $delete_query = "DELETE FROM `programs` WHERE `PROGRAM_USER` = '" . $_SESSION['username'] . "' AND `PROGRAM_NAME` = '" . mysqli_real_escape_string($con, $_POST['delete']) . "'";
...
{
    $disable_query = "UPDATE `programs` Set `DISABLED` = 'Yes' WHERE `PROGRAM_USER` = '" . $_SESSION['username'] . "' AND `PROGRAM_NAME` = '" . mysqli_real_escape_string($con, $_POST['disable']) . "'";
...
{
    $enable_query = "UPDATE `programs` Set `DISABLED` = 'No' WHERE `PROGRAM_USER` = '" . $_SESSION['username'] . "' AND `PROGRAM_NAME` = '" . mysqli_real_escape_string($con, $_POST['enable']) . "'";
        ...
        <div class="content">
            ...
            <div class="stats-grid">
                ...
                <div class="stat-card">
                    ...
                    <div class="stat-value"><?php echo htmlspecialchars($row[4]);?></div>
                    <div class="stat-icon">
            ...
            <!-- Create Program Banner -->
            <div class="card" style="background: linear-gradient(135deg, var(--primary) 0%, var(--primary-dark) 100%); color: white; border: none;">
                <div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 20px; padding: 20px;">
                    <div>
                        <h2 style="color: white; margin: 0 0 10px 0;">Protect Your Program</h2>
                        <p style="color: rgba(255,255,255,0.9); margin: 0;">License Your Program To Ensure No Unauthorised Access!</p>
                    </div>
                    <button class="btn btn-primary" id="createProgramBtn" style="background-color: white; color: var(--primary); font-weight: 600; border-color: white;">
            ...
            <div class="card">
                <div class="card-header">
                    ...
                    <div>Manage your licensed programs</div>
                </div>
                    ...
                    <div class="table-responsive">
                        <table>
                            ...
                            <tbody>
                                <?php if (empty($res_b)): ?>
                                    <tr>
                                        <td colspan="5" style="text-align: center; color: var(--text-muted);">No programs found. Create your first program!</td>
                                    </tr>
                                <?php else: ?>
                                    <?php foreach ($res_b as $program): ?>
                                        <tr>
                                            <td><?php echo htmlspecialchars($program['ID']); ?></td>
                                            <td><?php echo htmlspecialchars($program['PROGRAM_NAME']); ?></td>
                                            <td><?php echo htmlspecialchars($program['PROGRAM_ID']); ?></td>
                                            <td>
                                                <?php if($program['DISABLED'] == "No"): ?>
                                            ...
                                            <td>
                                                <div class="actions">
                                                    <form method="post" style="display: inline;">
                                                        <button type="submit" name="manage" value="<?php echo htmlspecialchars($program['PROGRAM_NAME']); ?>" class="btn btn-info btn-sm">
                                                    ...
                                                    <?php if ($program['DISABLED'] == "No"): ?>
                                                        <form method="post" style="display: inline;">
                                                            <button type="submit" name="disable" value="<?php echo htmlspecialchars($program['PROGRAM_NAME']); ?>" class="btn btn-warning btn-sm">
                                                                <i class="fas fa-pause"></i> <span>Disable</span>
                                                            </button>
                                                        </form>
                                                    <?php else: ?>
                                                        <form method="post" style="display: inline;">
                                                            <button type="submit" name="enable" value="<?php echo htmlspecialchars($program['PROGRAM_NAME']); ?>" class="btn btn-success btn-sm">
                                                                <i class="fas fa-play"></i> <span>Enable</span>
                                                            </button>
                                                        </form>
                                                    <?php endif; ?>

                                                    <form method="post" style="display: inline;" onsubmit="return confirm('Are you sure you want to delete this program?')">
                                                        <button type="submit" name="delete" value="<?php echo htmlspecialchars($program['PROGRAM_NAME']); ?>" class="btn btn-danger btn-sm">
                ...
                <form method="post">
                    <div class="modal-body">
                        <div class="form-group">
                            ...
                            <input type="text" name="programname" class="form-control" id="programname" required placeholder="Enter program name">
                        </div>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-light close-btn">Cancel</button>
                        <button type="submit" name="create" class="btn btn-primary">Create Program</button>
                    </div>
                </form>
            </div>
        </div>
