...
$mysqli = new mysqli("localhost", "u3230477_axisauth", "85K7AUXxsaPkcDS", "u3230477_axisauth");
...
if ($action == "create"){
    $key2 = key_gen();
    // Set expiry to 9999-12-31 instead of using type
    $expiry_date = '9999-12-31 23:59:59';
    mysqli_query($mysqli, 'INSERT INTO `license`(`KEY`, `DURATION`, `PROGRAM`, `LEVEL`, `USER_ID`, `EXPIRY`) VALUES (\''.$key2.'\', \''.$type2.'\', \''.$program.'\',  \''.$level.'\', \''.$row['ID'].'\', \''.$expiry_date.'\')');
...
if ($action == "bulk"){
    for($i=0;$i<$amount;$i++){
        $key2 = key_gen();
        // Set expiry to 9999-12-31 for bulk keys too
        $expiry_date = '9999-12-31 23:59:59';
        mysqli_query($mysqli, 'INSERT INTO `license`(`KEY`, `DURATION`, `PROGRAM`, `LEVEL`, `USER_ID`, `EXPIRY`) VALUES (\''.$key2.'\', \''.$type2.'\', \''.$program.'\',  \''.$level.'\', \''.$row['id'].'\', \''.$expiry_date.'\')');
        $list .= nl2br($key2. "\n");
    }
